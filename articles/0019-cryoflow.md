...
ログラスでは、社内のエンジニアがparquetについて触れている記事があります。
...
しかし、中身を手軽に編集できるツールには、期待どおりのものがありませんでした。
この不満が、cryoflowを作った一番の動機です。
ここで言う編集は、特定の列の値の一括変換や、列の追加・削除といった、列単位の操作です。
...
parquetをそのまま加工できる既存の手段は、DuckDBのようにSQLで操作するものが中心です。
...
そのため、ここではSQLを使わない選択をしました。

なお、DuckDBにもPythonからメソッドチェインで書けるRelational APIがあります。
ただ、開発当時はDuckDBを採用する方針では考えておらず、Polarsが技術的におもしろそうだという気持ちが先行していたため、Polars前提で設計しています。
...
そうなると、読み込み・変換・書き出しの骨組みは同じなのに、列名や係数だけが違うスクリプトがドメインごとに乱立するのではないか、というのが私の仮説でした。
