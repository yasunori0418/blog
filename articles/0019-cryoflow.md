...
parquetを直接読み書きして加工まで行えるCLIツールは、DuckDBのようにSQLで操作するものが中心です。
...
そうなると、ドメインごとに似たようなデータ処理スクリプトが乱立するのではないか、というのが私の仮説でした。
...
処理の骨組みはcryoflowが持ち、利用者はプラグインと設定ファイルだけを書く形になります。
