---
title: "VimConf 2025 SmallのLT振り返りと補足"
emoji: "🫖"
...
私は2023年から連続で参加しており、毎年楽しみにしているイベントです。
...
実は、LTプロポーザルを提出したのは期限最終日のことでした。
と言うのも、去年から今年にかけてVim活があまりできておらず、「自分には話すことがない」と登壇は諦めかけていました。

そんな中、vim-jpの`#event-vimconf`チャンネルでLTのプロポーザル投稿で盛り上がっているところを見て、「自分も何か喋れることがないか」と自分のdotfilesを読み漁ってみたところ、
...
これに対して「なんで…？」と聞かれても困る部分ではあり、どうしてそのような設計になっているのかは、Vimの作者であるBram Moolenaar氏（故人）に直接尋ねる以外、知る由はありません。
...
最初に提示したサンプルコードのような操作はできません。Vim9 scriptでは、非破壊的な値操作を心掛ける必要があります。
...
> 当時の私「Vim9 scriptの`a:`はパフォーマンスの問題で廃止されたのではなかったか？」
...
登壇の練習も事前に社内の10分勉強会で行っていたり、前日の段階でも数名に発表内容を見てもらったりして、フィードバックをもらえていたため、当日は**とても丁寧**な発表ができました。
...
また、この記事の中で`Vim9 script`の表記を統一している理由として、vimのhelpを日本語翻訳されている方からの[リプライ](https://x.com/YKirin0418/status/1984892220122808582)で正式な表記についての指摘があったためです。[^1]
これは、より正確な情報を届けるためにドキュメントの確認を徹底すべきであった点として、今後の改善点とします。
...
次のVimConfにもVim9 scriptの話をするか分かりませんが、またプロポーザルを出して登壇したい気持ちは強くなりました。
もっと人に語れるだけのVimのおもしろいエピソードや、Vimmerじゃない人達にも魅力を感じられるような内容を考えて、来年のVimConfに向けて温めておこうと思います！

[^1]: 一部で`vim9script`表記になっている部分がありますが、これは当時の内容を改変せず、あえてそのままにしています。
